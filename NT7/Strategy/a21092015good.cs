...
namespace NinjaTrader.Strategy{
    ...
    public class a21092015good : BTStrategy{
		...
		/*		
		private double Flatten2fdXXXX1 = Flatten2fdXXXX1();
		private double Flatten2fdXXXX2 = Flatten2fdXXXX2();
		private double Flatten2fdXXXX3 = Flatten2fdXXXX3();
		private double getFlatten2fd()
		{
			if (time === XXXX )
				return Flatten2fdXXXX1;
			
			else if (time === XXXX )
				return Flatten2fdXXXX2; 
			
			else
				return Flatten2fdXXXX3; 
		...
		*/
		...
		protected override void Init(){
			...
			if (Instrument.FullName.StartsWith("SI")){
				...
				this._eStartTime = ConvertZone("190000"); this._eFlatTime = ConvertZone("154500"); this._eEndTime = ConvertZone("160000");
			}
				...
				this._eStartTime = ConvertZone("083000"); this._eFlatTime = ConvertZone("214500"); this._eEndTime = ConvertZone("220000");			
			}
			...
			stoplosstab.Add("GC", 520.0);stoplosstab.Add("SI", 1300.0); stoplosstab.Add("6E", 350.0); stoplosstab.Add("CL", 520.0); stoplosstab.Add("FDAX", 1275.0);stoplosstab.Add("FGBL", 520.0); // give 2 tick leeway
	...
	//		#endregion
		...
		private void DSTAddMinus(int TimeShift, int phaseIndex){
			if (!this.daylightsavings() && phaseIndex == 1){	
				flattenclgcsi = ConvertZone("164500"); flatten2clgcsi = ConvertZone("170000"); flattenfd = ConvertZone("224500"); flatten2fd = ConvertZone("225900"); flatten6e = ConvertZone("164500"); flatten26e = ConvertZone("165900");
			...
			if ( phaseIndex == 2){
				flattenclgcsi = ConvertZone("154500"); flatten2clgcsi = ConvertZone("160000"); flattenfd = ConvertZone("214500"); flatten2fd = ConvertZone("215900"); flatten6e = ConvertZone("154500"); flatten26e = ConvertZone("155900");
				this._StartTime = this._StartTime - TimeShift; this._CutOffTime = this._CutOffTime - TimeShift; this._EndTime = this._EndTime - TimeShift;
			}
		}
...
//				}else //actually elseif
					...
					//	if ((Position.MarketPosition == MarketPosition.Flat && !isR2) || isR2)
						{
							...
							//if (BetQuantity > 0)
							{
								...
								//DoRoundNoMult(DoRoundMult(Range()[0]) / DoRoundMult(Range()[cells[cells.Length - 1] + 1]))
								//DoRoundNoMult(shortBollAvg[cells[cells.Length - 1]] / DoRoundMult(Range()[cells[cells.Length - 1]]))+
								//DoRoundNoMult(DoRoundMult(Range()[cells[cells.Length - 1]])/shortBollAvg[cells[cells.Length - 1]]  )
								DoRoundMult(Range()[0])+" "+ DoRoundMult(Open[cells[1]] - Close[cells[1]]) 
								);
								break;							
							}
						}
	...
	//						break;}
			...
			//				int BetQuantity = getBetQuantity(4,Volume[0]);
							...
							//if (BetQuantity > 0)
							{
								...
								Print("Signal Name: " + ActiveLabels[VersionLabel + "_" + fullBetQty] +  "	VersionLabel: " + VersionLabel + "	Date: " +  Time[0].ToString(" yyyy  MMM dd  HH:mm:ss" ) + " " +   
								//DoRoundNoMult(DoRoundMult(Range()[0]) / DoRoundMult(Range()[cells[cells.Length - 1] + 1]))
								//DoRoundNoMult(longBollAvg[cells[cells.Length - 1]] / DoRoundMult(Range()[cells[cells.Length - 1]]))+
								//DoRoundNoMult(DoRoundMult(Range()[cells[cells.Length - 1]])/longBollAvg[cells[cells.Length - 1]] )
								DoRoundMult(Range()[0])+" "+ DoRoundMult(Close[cells[1]] - Open[cells[1]]) 
	...
	//	#region Omit Lists
		...
		S00_OmitList = LoadOmitList (S00_OmitList, omitIndex);
			...
			//	result &= DoRoundMult(Close[0]) - DoRoundMult(Open[0]) > 0 ? 2 * DoRoundMult(Close[0]) - DoRoundMult(High[0]) <= 0 : true;
				...
				//result &= 2 * DoRoundMult(Close[0]) - DoRoundMult(Low[0]) >= 0 ;
			//	result &= DoRoundMult(Close[0]) - DoRoundMult(Open[0]) < 0 ? 2 * DoRoundMult(Close[0]) - DoRoundMult(Low[0]) >= 0 : true;
			}
...
//					result &= DoRoundMult(Bollinger(2,14).Upper[cells[cells.Length - 1]]) - DoRoundMult(Bollinger(2,14).Middle[cells[cells.Length - 1]]) >= 
//							  DoRoundMult(Bollinger(2,14).Upper[cells[cells.Length - 1]+14]) - DoRoundMult(Bollinger(2,14).Middle[cells[cells.Length - 1]+14]);	
...
//					result &= DoRoundMult(Bollinger(2,14).Middle[cells[cells.Length - 1]]) - DoRoundMult(Bollinger(2,14).Lower[cells[cells.Length - 1]]) >= 
//							  DoRoundMult(Bollinger(2,14).Middle[cells[cells.Length - 1]+14]) - DoRoundMult(Bollinger(2,14).Lower[cells[cells.Length - 1]+14]);
...
//						result &= (DoRoundMult(Open[0]) - DoRoundMult(EMA(3)[0]) > 0);
			...
			result &= DoRoundMult(ATR(14)[0]) >= 2 && DoRoundMult(Range()[0]) >= 3; //already >=3
		...
		S2_OmitList = LoadOmitList (S2_OmitList, omitIndex);
			...
			for (int i = 0; i < cells.Length - 1; i++){
				if (isLong){
					...
					//result &= DoRoundNoMult(longBollAvg[cells[cells.Length - 1] + 1]) <= DoRoundMult(Bollinger(2,14).Upper[cells[cells.Length - 1]]) - DoRoundMult(Bollinger(2,14).Middle[cells[cells.Length - 1]]);
				}
				else{	
					...
					//result &= DoRoundNoMult(shortBollAvg[cells[cells.Length - 1] + 1]) <= DoRoundMult(Bollinger(2,14).Middle[cells[cells.Length - 1]]) - DoRoundMult(Bollinger(2,14).Lower[cells[cells.Length - 1]]);
				}
			}
			return result;		
		}
...
//					result &= DoRoundMult(Range()[0]) < 34;
//					result &= DoRoundMult(Range()[0]) > 21 ? DoRoundMult(Close[cells[1]] - Open[cells[1]]) > 11 : true; 
//					result &= DoRoundMult(Close[cells[1]] - Open[cells[1]]) >= 2 ? DoRoundMult(Range()[0]) < 5*DoRoundMult(Close[cells[1]] - Open[cells[1]])  : true; 
//					result &= DoRoundMult(Range()[0]) + 2 > DoRoundMult(Close[cells[1]] - Open[cells[1]]) ; 
...
//				result &= ((DoRoundMult(Close[cells[cells.Length-1]+1]) > DoRoundMult(Open[cells[cells.Length-1]+1])) ?
//						   ((DoRoundMult(Close[cells[cells.Length - 1]]) - DoRoundMult(Open[cells[cells.Length - 1]])) < 10 * (DoRoundMult(Close[cells[cells.Length-1]+1]) - DoRoundMult(Open[cells[cells.Length-1]+1]))) : true);
			}	
			else{	
					result &= DoRoundMult(Range()[0]) < (4 + Convert.ToInt32(cells[cells.Length - 1])) * DoRoundMult(Range()[cells[cells.Length - 1] + 1]) -1 ;
				
//					result &= DoRoundMult(Range()[0]) < 34;
//					result &= DoRoundMult(Range()[0]) > 21 ? DoRoundMult(Open[cells[1]] - Close[cells[1]]) > 11 : true; 
//					result &= DoRoundMult(Open[cells[1]] - Close[cells[1]]) >= 2 ? DoRoundMult(Range()[0]) < 5*DoRoundMult(Open[cells[1]] - Close[cells[1]])  : true; 
//					result &= DoRoundMult(Range()[0]) + 2 >  DoRoundMult(Open[cells[1]] - Close[cells[1]]); 
...
//				result &= ((DoRoundMult(Open[cells[cells.Length-1]+1]) > DoRoundMult(Close[cells[cells.Length-1]+1])) ?
//						   ((DoRoundMult(Open[cells[cells.Length - 1]]) - DoRoundMult(Close[cells[cells.Length - 1]])) < 10 * (DoRoundMult(Open[cells[cells.Length-1]+1]) - DoRoundMult(Close[cells[cells.Length-1]+1]))) : true);
			}
			return result;	
		}
...
//				}
		...
		if (!exitbegun
				...
				//	&& DoRoundNoMult(ADX(14)[2]) - getUniqueFunctions(UniqueFunctionType.DI, isLong, new String[]{"2"}) < 0	
...
//			        (getDirection(isLong) * (DoRoundMult(Close[1]) - DoRoundMult(Open[1])) > 2 && getDirection(isLong) * (DoRoundMult(Close[1]) - DoRoundMult(Open[1])) >= 2 * getDirection(isLong) * (DoRoundMult(Close[2]) - DoRoundMult(Open[2]))  ) : true)
//				&& ((BarsSinceEntry() >= 20 && getDirection(isLong) * DoRoundMult(Close[0]) > getDirection(isLong) * DoRoundMult(Open[BarsSinceEntry()]) + 1 && getDirection(isLong) * (DoRoundMult(Close[2]) - DoRoundMult(Open[2])) > 0 ) ? 
//			        (getDirection(isLong) * (DoRoundMult(Close[1]) - DoRoundMult(Open[1])) > 2 && getDirection(isLong) * ((DoRoundMult(Close[1]) - DoRoundMult(Open[1])) - (DoRoundMult(Close[2]) - DoRoundMult(Open[2]))) > 0) : true)
//	
...
//						getDirection(isLong) * (DoRoundMult(Close[1]) - DoRoundMult(Open[1])) >= getDirection(isLong) * (DoRoundMult(Open[2]) - DoRoundMult(Close[2]))) : true)
				...
				&& (DoRoundMult(Range()[1]) >= DoRoundMult(Range()[2]) )	
				...
				//&& ((BarsSinceEntry() < 20 && getDirection(isLong) * (DoRoundMult(Open[2]) - DoRoundMult(Close[2])) < 0)	? 
			      //  getDirection(isLong) * (DoRoundMult(Close[0]) - DoRoundMult(Open[2])) < 0 : 
				    //getDirection(isLong) * (DoRoundMult(Close[0]) - DoRoundMult(Open[2])) <= 0)
		//	&&  DoRoundMult(Range()[1]) < 2 * getDirection(isLong) * (DoRoundMult(Close[1]) - DoRoundMult(Open[1]))
				&& DoRoundMult(Range()[1]) >= DoRoundMult(ATR(14)[1])
				&& DoRoundMult(Range()[0]) >= DoRoundMult(ATR(14)[0])
				&& getDirection(isLong) * (DoRoundMult(Close[1]) - DoRoundMult(Open[1])) > 0
				&& getDirection(isLong) * (DoRoundMult(Open[0]) - DoRoundMult(Close[0])) > 2	//!!!
					&& (getDirection(isLong) * (DoRoundMult(Open[2]) - DoRoundMult(Close[2])) < 0 ? DoRoundMult(ATR(14)[1]) > DoRoundMult(ATR(14)[2]) : true)
...
//					&& (getDirection(isLong) * (DoRoundMult(Close[0]) - DoRoundMult(Open[1])) < 0 || 
//						(
//						getDirection(isLong) * (DoRoundMult(Close[1]) - DoRoundMult(EMA(3)[1])) > -1 
//						&& getDirection(isLong) * (getUniqueFunctions(UniqueFunctionType.HIGHLOW, !isLong, new string[]{"1"}) - DoRoundMult(Close[0])) >= 0
//						)
//					)
...
//					&& getDirection(isLong) * (DoRoundMult(Close[2]) - DoRoundMult(Open[2])) <= 0
			...
			if (!exitbegun
				...
				&& getDirection(isLong) * (DoRoundMult(EMA(3)[2]) - DoRoundMult(EMA(14)[2])) <= 0 //new
			...
			if (!exitbegun
				...
				&& (getDirection(isLong) * 5 * (DoRoundMult(Close[0]) - getUniqueFunctions(UniqueFunctionType.HIGHLOW, !isLong, new string[]{"0"})) >= DoRoundMult(4 * Range()[0]) ||
				   getDirection(isLong) * (DoRoundMult(Open[0]) - getUniqueFunctions(UniqueFunctionType.BOLLINGER, isLong, new String[]{"2","14","0"})) < -1)
				
//				&& ((getDirection(isLong) * (DoRoundMult(Close[0]) - DoRoundMult(Open[BarsSinceEntry()])) > 0 && getDirection(isLong) * (DoRoundMult(Open[0]) - DoRoundMult(Open[BarsSinceEntry()])) > 0) ||
//				   	(getDirection(isLong) * (DoRoundMult(Close[0]) - DoRoundMult(Open[BarsSinceEntry()])) <= 0 && getDirection(isLong) * (DoRoundMult(Open[0]) - DoRoundMult(Open[BarsSinceEntry()])) <= 0))
			){
				int LookBackBars = 3;
				if (EXIT_TIME_BREAK_CHECKER (LookBackBars) && (isLong && Position.MarketPosition == MarketPosition.Long || !isLong && Position.MarketPosition == MarketPosition.Short))
...
//				getDirection(isLong) * (DoRoundNoMult(4 * (getUniqueFunctions(UniqueFunctionType.HL, isLong, null) - DoRoundMult(Open[BarsSinceEntry()]) - 1)))	
			...
			if (!exitbegun
				//&& ((ToTime(Time[0]) >= getFlatten2fd() && ToTime(Time[0]) <= getFlatten2fd() && Instrument.FullName.StartsWith("FDAX")) ||
		...
		protected bool zStop(int cellone, int celltwo, int version, bool isLong, bool isR2){ 
			...
			if (BarsSinceEntry() >= 1	
				...
				&& Math.Truncate(ADX(14)[celltwo]) - Math.Truncate(ADX(14)[0]) >= Convert.ToInt32(celltwo) -1 //>
					//&& getDirection(isLong) * (DoRoundMult(Open[celltwo]) - DoRoundMult(Open[BarsSinceEntry()])) >= 0
				...
				&& getDirection(isLong) * (DoRoundMult(EMA(3)[0]) - DoRoundMult(EMA(35)[0])) <=0
			){
